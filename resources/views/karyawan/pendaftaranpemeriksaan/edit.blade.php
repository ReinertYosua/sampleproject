          ...
          <div class="card">
            <div class="card-body">
              ...
              @include('partial.dangeralert')
                <form id="formPemeriksaan" action="{{ route('karyawan.pendaftaranpemeriksaan.update', $getDaftarPemeriksaan[0]->id) }}" method="post" class="row g-3" enctype="multipart/form-data">
                  @csrf
                  @method('PUT')
                    <div class="col-md-6">
                        ...
                        <input type="text" id="namaPasien" name="namaPasien" value="{{ $getDaftarPemeriksaan[0]->name }}" class="form-control @error('namaPasien') is-invalid @enderror" autocomplete="off" disabled>
                   
                    </div>
                
                    <div class="col-md-6">
                        <label for="" class="form-label">Nama Dokter Pemberi Rekomendasi</label>
                        <input type="text" id="namaDokterPengirim" name="namaDokterPengirim" value="{{ old('namaDokterPengirim', $getDaftarPemeriksaan[0]->namaDokterPengirim) }}" class="form-control @error('namaDokterPengirim') is-invalid @enderror" autocomplete="off" disabled>
                        
                    </div>
                    <div class="col-md-6">
                        <label for="" class="form-label">File Lampiran</label>
                        <a href="{{ route('karyawan.file.download', basename($getDaftarPemeriksaan[0]->fileLampiran)) }}" class="btn btn-primary">Download {{ basename($getDaftarPemeriksaan[0]->fileLampiran) }}</a>
                                ...
                                <tbody>
                                    ...
                                    @foreach($getDetailPendaftaran as $gdp)
                                        <tr id="daftar{{ $index }}" data-row="{{ $index }}">
                                            ...
                                            <td>
                                                <input type="text" value="{{ $gdp->harga }}" name="harga[]" id="harga{{ $index }}" class="price form-control @error('harga.{{ $index }}') is-invalid @enderror" autocomplete="off" readonly> 
                                            </td>
                                            <td>
                                                <select id="status{{ $index }}" name="status[]" class="form-control @error('status.{{ $index }}') is-invalid @enderror">
                                                    <option value="" selected>- Pilih -</option>
                                                    <option value="ya" {{ $gdp->statusKetersediaan =='ya' ? 'selected':''  }}>Ya</option>
                                                    <option value="tidak" {{ $gdp->statusKetersediaan =='tidak' ? 'selected':''  }}>Tidak</option>
                                                </select> 
                                            </td>
                                            <td>
                                                <textarea name="keterangan[]" id="keterangan{{ $index }}" class="form-control @error('keterangan.{{ $index }}') is-invalid @enderror">{{ $gdp->keterangan }}</textarea>
                                            </td>
