            ...
            <div class="card border-0 shadow rounded">
                <div class="card-body">
                    ...
                    <div class="container">
                        <div class="row">
                            <div class="col-lg-12">
                            <a href="{{ route('pendaftaranpemeriksaan.create') }}" class="btn btn-md btn-success mb-3 float-end"><i class="bi bi-plus-lg"></i> Buat Pendaftaran Pemeriksaan</a>
                            </div>
                        </div>
                    </div>
                        ...
                        <tbody>
                            @foreach($datadaftarperiksa as $dp)
                                <tr>
                                    ...
                                    <td><a href="{{ route('file.download', basename($dp->fileLampiran)) }}" class="btn btn-sm btn-primary"><i class="bi bi-cloud-download"></i> {{ basename($dp->fileLampiran) }}</a></td>
                                    <td>
                                        <div class="btn-group" role="group" aria-label="Aksi">
                                            <a href="{{ route('pendaftaranpemeriksaan.edit', $dp->id)  }}" class="btn btn-sm btn-primary me-2"><i class="bx bx-pencil"></i></a>
                                            ...
                                            <form onsubmit="return confirm('Apakah Anda Yakin?')" action="{{ route('pendaftaranpemeriksaan.destroy', $dp->id)  }}" method="POST">
                                                @csrf
                                                @method('delete')
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        // $('#tabel_detail_pendaftaran').DataTable();
        ...
        $('.detailBtn').click(function(e) {
                ...
                var id = $(this).data('id');
                 ...
                 $('#tgldaftar').empty();
                 $('#noregistrasi').empty();
                 $('#namadokterrekomendasi').empty();

                $.ajax({
                    url: "{{ route('pendaftaranpemeriksaan.show', ':id') }}".replace(':id', id),
                    dataType: 'json',
